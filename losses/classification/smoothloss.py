...
class SmoothedCrossEntropy(Module):
    ...
    def forward(self, output: Tensor, label: Tensor) -> Tensor:
        ...
        smoothed_label = self.smooth_label(
            label, output.shape[1]).to(output.dtype)
        output = F.log_softmax(output, 1)
        loss = -output * smoothed_label
        if self.weight is not None:
            weight_shape = [1, self.weight.shape[0]] + \
                [1 for _ in range(len(output.shape) - 2)]
...
def to_onehot(label: Tensor, n_class: Optional[int] = None) -> Tensor:
    ...
    label_oh = torch.zeros([label.shape[0], n_class] +
                           list(label.shape)[1:]).long().to(label.device)
